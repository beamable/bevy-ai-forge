/*
 * Auth Actor
 *
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.0
 * Contact: support@beamable.com
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct StatSubscriptionNotification {
    #[serde(rename = "statsBefore")]
    pub stats_before: std::collections::HashMap<String, String>,
    #[serde(rename = "statsAfter")]
    pub stats_after: std::collections::HashMap<String, String>,
}

impl StatSubscriptionNotification {
    pub fn new(stats_before: std::collections::HashMap<String, String>, stats_after: std::collections::HashMap<String, String>) -> StatSubscriptionNotification {
        StatSubscriptionNotification {
            stats_before,
            stats_after,
        }
    }
}

