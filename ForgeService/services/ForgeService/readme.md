...
/project
  ...
  /services
     /ForgeService
        ...
        ForgeService.cs
     /ForgeServiceLibrary
        ForgeServiceLibrary.csproj
        ForgeServiceLibrary.cs
```

## Running Locally
The ForgeService project can be run locally by starting it from an IDE, or by executing the `dotnet run` command.
...
```shell
beam services deploy --ids ForgeService
```

### MSBuild Properties
The `ForgeService.csproj` file has a few configuration options that can be set.
```xml
<PropertyGroup>
...
#### BeamableTool
By default, both ForgeService and ForgeServiceLibrary have the BeamCLI installed as a project tool. 
...
remote swagger. 
```shell
beam project open-swagger ForgeService
```
...
at each linked Unity project's `Assets/Beamable/Microservice/Microservices/ForgeServiceClient.cs`. 

Additionally, the `share-code` msbuild target in the ForgeServiceLibrary project will copy their
