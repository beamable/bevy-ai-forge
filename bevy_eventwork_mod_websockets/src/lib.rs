...
pub struct BeamableNetworkPacket {
    kind: String,
    data: Vec<u8>,
}
...
mod native_websocket {
    ...
    use crate::{BeamableNetworkPacket, BeamableWasmNotification};
                        ...
                        if let Ok(Ok(packet)) = serde_json::from_str::<BeamableWasmNotification>(&t)
                            .map(|s| {
                                ...
                                let data = s
                                    ...
                                    .as_bytes()
                                    .to_vec();
                                let beam = BeamableNetworkPacket { kind, data };
                                if let Ok(Ok(s)) =
