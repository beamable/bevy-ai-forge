pub mod macros {
            ...
            impl bevy::ecs::world::Command for $command {
                fn apply(self, world: &mut World) {
                    ...
                    let context = world
                        ...
                        .unwrap();
                    let key = world.get_resource::<$crate::config::BeamableConfig>().unwrap().get_x_beam_scope();
    ...
    #[macro_export]
    macro_rules! create_request_with_args {
        (
            $task_name: ident,
            $command: ident,
            $event_completed_name: ident,
                ...
                fn apply(self, world: &mut World) {
                    ...
                    thread_pool
                        .spawn(async move {
                            ...
                            #[cfg(not(target_family = "wasm"))]
                            let r = async_compat::Compat::new(async {
                                $call_from_open_api(&config, request_data)
                                    .await
                            })
                            .await;
                            println!("{:#?}", r);
    ...
    pub use create_request_with_args;
}
